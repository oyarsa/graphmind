"""Fine-tune a causal LM with LoRA for rationale + rating generation using graph context.

This is an extension of sft_gen.py that uses pre-computed graph and related paper
context from PaperWithGraphContext data (produced by sft_gen_preprocess.py).

Input: Paper title, abstract, knowledge graph, and related papers
...
from paper.gpt.model import PaperWithGraphContext
...
app = typer.Typer(
    ...
    help="Fine-tune Llama with graph context for rationale + rating generation.",
)
...
def format_input(
    ...
    """Format the input prompt for the model with graph context."""
    return INPUT_TEMPLATE.format(
        title=title,
        abstract=abstract,
        graph=graph if graph else "(No graph available)",
        positive=positive if positive else "(No supporting papers)",
        negative=negative if negative else "(No contrasting papers)",
    )
...
def format_training_example(
    ...
    input_text = format_input(title, abstract, graph, positive, negative)
    output_text = format_output(rationale, rating)
    return FULL_TEMPLATE.format(input=input_text, output=output_text)


def preprocess_dataset(dataset: list[PaperWithGraphContext]) -> Dataset:
    ...
    for item in dataset:
        ...
        text = format_training_example(
            ...
            graph=item.graph_text,
            positive=item.positive_related,
...
def evaluate_model(
    ...
    original_data: list[PaperWithGraphContext],
    output_dir: Path,
    config: AppConfig,
) -> list[PaperEvaluated]:
    ...
    for i, item in enumerate(original_data):
        ...
        # Format input with graph context
...
def load_dataset_from_file(
    ...
    n: int | None,
    rng: random.Random,
) -> tuple[Dataset, list[PaperWithGraphContext]]:
    ...
    """
    data = load_data(file, PaperWithGraphContext)
...
def train(
    ...
    """Fine-tune a causal LM with LoRA for rationale + rating generation with graph context.

    Uses paper title, abstract, knowledge graph, and related papers as input. Trains
    the model to generate a review rationale followed by a rating (1-5).
...
def infer(
    ...
    """Run inference using a trained generative model on a dataset with graph context.

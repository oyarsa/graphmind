...
def _augment_asap(
    ...
    `min_references`.
    """
    augmented_papers: list[ASAPWithFullS2] = []
    s2_papers_from_query = {_clean_title(s2.title_query): s2 for s2 in s2_papers}

    for asap_paper in asap_papers:
        s2_references = [
            s2
            for ref in asap_paper.references
            if (s2 := s2_papers_from_query.get(_clean_title(ref.title)))
        ]
...
def _filter_recommended(
    ...
    """
    asap_titles = {_clean_title(paper.title) for paper in asap_papers}
    return [
        rec
        for rec in recommended_papers
        if any(
            _clean_title(source) for source in rec.sources_asap if source in asap_titles
        )
    ]


def _unique_asap_refs(asap_papers: Iterable[ASAPWithFullS2]) -> list[asap.S2Paper]:
    ...
    for asap_paper in asap_papers:
        for ref in asap_paper.references:
            ref_title = _clean_title(ref.title_query)

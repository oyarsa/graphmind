  ...
  return (
    <div className="border rounded-lg mb-3 bg-white">
      ...
      <div className={`collapsible-content ${isOpen ? 'open' : ''}`}>
        <div className="px-4 py-4 border-t">{children}</div>
      </div>
    </div>
  );
}
...
// Get paper annotation status: 'done', 'incomplete', or 'todo'
function getPaperStatus(ann, paper) {
  if (!ann) return 'todo';
  if (ann.done) return 'done';

  const hasAnyChecked = (ann.claims || []).some(v => v) ||
                        (ann.methods || []).some(v => v) ||
                        (ann.experiments || []).some(v => v);
  const hasRating = ann.rationaleQuality != null;

  if (!hasAnyChecked && !hasRating) return 'todo';
  return 'incomplete';
}

// Rating radio buttons
function RatingInput({ value, onChange }) {
  const labels = ['1 - Not useful', '2 - Marginally', '3 - Somewhat', '4 - Useful', '5 - Very useful'];
  return (
...
function App() {
  ...
  // Warn before leaving if papers are loaded
  useEffect(() => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-4 flex items-center gap-4">
        ...
        >
          {papers.map((p, idx) => {
            ...
            const status = getPaperStatus(annotations[idx], p);
            const indicator = status === 'done' ? '✓' : status === 'incomplete' ? '◐' : '○';
            return (
              <option key={idx} value={idx}>
                {indicator} {idx + 1}. {title.substring(0, 60)}{title.length > 60 ? '...' : ''}
              </option>
            );
          })}
        </select>
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-4">
        ...
        {/* Rationale Quality - directly below rationale */}
        <div className="mt-4 p-4 bg-gray-50 rounded-lg border">
          <h4 className="font-medium text-gray-700 mb-2">Rate Rationale Usefulness</h4>
          <RatingInput
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Graph Components</h3>
        <p className="text-sm text-gray-500 mb-4">
          Check items that are accurately extracted from the paper.
        </p>
